...
class IntegratedClimateAnalyzer:
    ...
    def analyze(
        ...
        if use_multisource:
            ...
            quality = DataQualityAnalyzer.assess(df)
    ...
    def _build_explainability(self, window_df: pd.DataFrame, recent_days: int = 14) -> Dict:
        ...
        # Últimos N días del dataset disponible (no del año objetivo si está fuera del rango)
        ...
        # Drivers del modelo (top features)
        feats = AdvancedClimatePredictor().create_features(window_df)
...
def _cli():
    ...
    # métricas de calidad de ensamble (si multisource)
    if dq:
