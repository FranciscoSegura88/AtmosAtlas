...
DEFAULT_PARAMS = [
    ...
    "ALLSKY_SFC_SW_DWN", # radiación solar - shortwave down (W/m2)
...
def fetch_power_point(lat, lon, start, end, parameters=DEFAULT_PARAMS, community="AG", retries=5, backoff=2):
    ...
    while attempt < retries:
        try:
            ...
            r.raise_for_status()
            data = r.json()
            # la API devuelve 'properties' -> 'parameter' -> {param: {YYYYMMDD: value}}
            return data
...
def power_json_to_dataframe(power_json, parameters=DEFAULT_PARAMS):
    ...
    # find all dates from any parameter
    dates = set()
    ...
    rows = []
    for d in dates:
        # POWER date format is YYYYMMDD
        dt = datetime.strptime(d, "%Y%m%d").date()
        row = {"date": dt}
        for p in parameters:
            # Keep None for missing so we can convert later
    ...
    # convert numeric columns to floats (coerce errors to NaN)
...
def main():
    ...
    # guarda CSV con nombre informativo si no se dio --out
    if args.out:
        out_fn = args.out
    else:
    ...
    # también muestra primeras 5 filas
    print(df.head())
